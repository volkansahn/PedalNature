...
final class CameraViewController: UIViewController {
    ...
    private let resumeButton : UIButton = {
        let button = UIButton()
        button.isHidden = true
        button.setTitle("Tap to Resume", for: .normal)
        return button
    }()
    ...
    override func viewDidLoad() {
        ...
        resumeButton.addTarget(self,
                                    action: #selector(resumeButtonPressed), for: .touchUpInside)
    ...
    override func viewDidLayoutSubviews() {
        ...
        resumeButton.center = view.center
    ...
    @objc func takePicture(){
        ...
        sessionQueue.async {
            ...
            let photoCaptureProcessor = PhotoCaptureProcessor(with: photoSettings, willCapturePhotoAnimation: {
                ...
                self.sessionQueue.async {
                    self.imageList.append(photoCaptureProcessor.routeImage!)
    ...
    private func addObservers() {
        let keyValueObservation = session.observe(\.isRunning, options: .new) { _, change in
            ...
            DispatchQueue.main.async {
                ...
                self.takePictureButton.isEnabled = isSessionRunning
                //self.recordButton.isEnabled = isSessionRunning && self.movieFileOutput != nil
                //self.photoButton.isEnabled = isSessionRunning
                self.captureModeControl.isEnabled = isSessionRunning
                self.changeCameraButton.isEnabled = isSessionRunning
...
extension CameraViewController: AVCaptureFileOutputRecordingDelegate{
    ...
    func fileOutput(_ output: AVCaptureFileOutput, didStartRecordingTo fileURL: URL, from connections: [AVCaptureConnection]) {
        ...
        DispatchQueue.main.async {
            self.recordVideoButton.isEnabled = true
            self.recordVideoButton.setImage(UIImage(systemName: "stop.fill"), for: [])
        }
    }
    ...
    func fileOutput(_ output: AVCaptureFileOutput,
        ...
        DispatchQueue.main.async {
            ...
            self.recordVideoButton.isEnabled = true
            self.captureModeControl.isEnabled = true
            self.recordVideoButton.setImage(UIImage(systemName: "record.circle"), for: [])
        }
    }
